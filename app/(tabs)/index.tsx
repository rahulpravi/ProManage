...
export default function DashboardScreen() {
  ...
  const [stockModalMachine, setStockModalMachine] = useState<string | null>(
    null,
  );
  ...
  const buildWeeklyData = (logs: ProductionLog[]) => {
    ...
    logs.forEach((l) => {
      ...
      const machineIdx = DEFAULT_MACHINES.findIndex(
        (m) => m.name === l.machineName,
      );
  ...
  const buildMonthlyData = (prodLogs: ProductionLog[], saleLogs: SaleLog[]) => {
    ...
    prodLogs.forEach((l) => {
      ...
      if (d.getFullYear() === year && d.getMonth() === month) {
        prod[d.getDate() - 1] += l.quantity;
      }
    });
    saleLogs.forEach((l) => {
      const d = new Date(l.soldAt);
      if (d.getFullYear() === year && d.getMonth() === month) {
        sale[d.getDate() - 1] += l.quantity;
      }
    });
    setMonthlyDays(days);
    setMonthlyProd(prod);
  ...
  const modalMachineStock = stockModalMachine
    ? stats.stock.filter((s) => s.machineCategory === stockModalMachine)
    : [];
    ...
    <View style={[s.container, { paddingTop: topPad }]}>
      ...
      >
        <View style={s.summaryRow}>
          {[
            ...
            <View key={item.label} style={s.summaryCard}>
              <View
                style={[s.summaryIcon, { backgroundColor: item.color + "22" }]}
              >
                <Ionicons
            ...
            {DEFAULT_MACHINES.map((m, i) => (
              <View key={m.id} style={s.legendItem}>
                <View
                  style={[
                    s.legendDot,
                    { backgroundColor: colors.machineColors[i] },
                  ]}
                />
        ...
        <View style={s.chartCard}>
          ...
          <View style={s.legendRow}>
            <View style={s.legendItem}>
              <View
                style={[s.legendDot, { backgroundColor: colors.primaryLight }]}
              />
              <Text style={s.legendText}>Production</Text>
            </View>
            <View style={s.legendItem}>
              <View
                style={[s.legendDot, { backgroundColor: colors.warning }]}
              />
        ...
        {stats.machineStats.map((ms, idx) => {
          ...
          const firstPartName = machineParts[0] ?? "Part";
          const machineStockItems = stats.stock.filter(
            (st) => st.machineCategory === ms.machine.name,
          );
            ...
            <View key={ms.machine.id} style={s.machineCard}>
              <View style={s.machineHeader}>
                <View
                  style={[
                    s.machineIcon,
                    { backgroundColor: colors.machineColors[idx] + "33" },
                  ]}
                >
                  <Ionicons
                    name="hardware-chip"
                    size={18}
                    color={colors.machineColors[idx]}
                  />
                </View>
                ...
                <TouchableOpacity
                  style={[
                    s.machineBadge,
                    { backgroundColor: colors.machineColors[idx] + "22" },
                  ]}
              ...
              <View style={s.machineStats}>
                <View style={[s.machineStat, { flex: 1.2 }]}>
                  <Text style={s.machineStatValue} numberOfLines={1}>
                    {firstPartName}
                  </Text>
                  <Text style={s.machineStatLabel}>Main Part</Text>
                </View>
                {[
      ...
      <Modal visible={!!stockModalMachine} animationType="slide" transparent>
        <View style={s.modalOverlay}>
          <View style={s.modalSheet}>
            ...
            {modalMachineStock.length === 0 ? (
              <View style={s.modalEmpty}>
                <Ionicons
                  ...
                  color={colors.textMuted}
                />
                <Text style={s.modalEmptyText}>No stock for this machine</Text>
              </View>
            ) : (
              modalMachineStock.map((item) => (
                <View key={item.id} style={s.stockRow}>
                  <View style={s.stockRowLeft}>
                    <Ionicons name="cube" size={16} color={colors.primary} />
                    <Text style={s.stockRowName}>{item.partName}</Text>
                  </View>
                  <View
                    style={[
                      s.stockQtyBadge,
            ...
            <View style={{ height: 8 }} />
            <Text style={s.allPartsLabel}>
              All Parts for {stockModalMachine}
            </Text>
            {(MACHINE_PARTS[stockModalMachine ?? ""] ?? []).map((partName) => {
              const found = modalMachineStock.find(
                (s) => s.partName === partName,
              );
              return (
                <View key={partName} style={s.partRow}>
                  <Text style={s.partRowName}>{partName}</Text>
                  <Text style={s.partRowQty}>
                    {found ? `${found.quantity} units` : "Not in stock"}
                  </Text>
                </View>
              );
            })}
...
function makeStyles(colors: any) {
  return StyleSheet.create({
    ...
    modalEmpty: { alignItems: "center", paddingVertical: 20, gap: 8 },
    modalEmptyText: {
      fontSize: 14,
      fontFamily: "Inter_400Regular",
      color: colors.textMuted,
