...
import { DEFAULT_MACHINES } from "@/lib/database";
...
export default function StockScreen() {
  ...
  const [selectedMachine, setSelectedMachine] = useState(DEFAULT_MACHINES[0].category);
  ...
  const handleAdd = async () => {
    ...
    setSubmitting(true);
    try {
      await addStock(partName.trim(), selectedMachine, qty);
      await refreshStock();
          ...
          <View style={styles.modalSheet}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.chipScroll}>
              {DEFAULT_MACHINES.map((m) => (
                <TouchableOpacity
                  key={m.id}
                  style={[styles.chip, selectedMachine === m.category && styles.chipActive]}
                  onPress={() => setSelectedMachine(m.category)}
                >
                  <Text style={[styles.chipText, selectedMachine === m.category && styles.chipTextActive]}>
                    {m.name}
                  </Text>
                </TouchableOpacity>
              ))}
